# OSINTview — API Reference

> Comprehensive reference for all Vercel Edge Function endpoints powering the OSINTview intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://osintview.app/api/earthquakes`).
...
OSINTview exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `osintview\.app$` | `https://osintview.app` |
| 2 | `\.osintview\.app$` | `https://*.osintview.app` |
...
OSINTview uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
