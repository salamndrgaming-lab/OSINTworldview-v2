# Component Documentation — OSINTview
...
OSINTview's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['osintview-panel-spans']` as a JSON map of `{ [id]: height }`.
...
OSINTview ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['osintview_recent_searches']`
(most recent 10).
