...
OSINTview is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
Stored in `localStorage` under `osintview-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `osintview-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `osintview-monitors` | `Monitor[]` | Color/label configs for monitors |
| `osintview-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `osintview-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('osintview-theme', 'light')
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates osintview_db v1
...
- **Toggles key:** `osintview-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by OSINTview:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `osintview-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `osintview-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `osintview-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `osintview-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `osintview-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `osintview-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `osintview-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `osintview-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `osintview-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `osintview-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `osintview-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
