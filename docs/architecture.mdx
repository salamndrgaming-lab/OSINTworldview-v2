...
description: "Design principles, intelligence tradecraft, algorithmic decisions, and implementation patterns behind OSINTview."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/salamndrgaming-lab/OSINTworldview-v2/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
OSINTview is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All five dashboard variants (OSINTview, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:

| Hostname | Variant |
| --- | --- |
| `tech.osintview.app` | `tech` |
| `finance.osintview.app` | `finance` |
| `commodity.osintview.app` | `commodity` |
| `happy.osintview.app` | `happy` |
| `osintview.app` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['osintview-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `osintview.app`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`osintview@{version}`) enables regression detection across deployments.
