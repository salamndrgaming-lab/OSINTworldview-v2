---
title: "Changelog"
description: "All notable changes to OSINTview, organized by release."
rss: true
---

All notable changes to OSINTview are documented here. Subscribe via [RSS](/changelog/rss.xml) to stay updated.
...
- Story header uses full domain OSINTVIEW.APP (#799)
...
- **Edge caching**: Cloudflare edge caching for proxy.osintview.app (#478) and api.osintview.app (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.osintview.app`) with finance/trading-focused feeds, panels, and map defaults
...
- **CORS hardening**: Tighten Vercel preview deployment regex to block origin spoofing (`osintviewEVIL.vercel.app`)
