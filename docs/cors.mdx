...
| Pattern | Matches |
| --- | --- |
| `(*.)?osintview.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `osintview-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-OSINTview-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
