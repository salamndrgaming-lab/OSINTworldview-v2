...
description: "OSINTview exposes two health endpoints for monitoring data pipeline integrity. Both run on the Vercel Edge Runtime and query Redis (Upstash) to assess system state."
---
...
```bash
# Full health check
curl -s https://api.osintview.app/api/health | jq .

# Compact (problems only)
curl -s "https://api.osintview.app/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://api.osintview.app/api/health
...
```bash
curl -s https://api.osintview.app/api/seed-health \
  -H "Origin: https://osintview.app" | jq .
```
...
STATUS=$(curl -s "https://api.osintview.app/api/health?compact=1")
