...
const ALLOWED_ENV_KEYS = new Set([
  ...
  'OLLAMA_API_URL', 'OLLAMA_MODEL', 'OSINTVIEW_API_KEY', 'WTO_API_KEY',
...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  headers.set('Origin', 'https://osintview.app');
  return fetch(target, {
    method: req.method,
    headers,
    body,
...
function resolveConfig(options = {}) {
  ...
  const remoteBase = String(options.remoteBase ?? process.env.LOCAL_API_REMOTE_BASE ?? 'https://api.osintview.app').replace(/\/$/, '');
...
const SIDECAR_ALLOWED_ORIGINS = [
  ...
  // Only allow exact domain or single-level subdomains (e.g. preview-xyz.osintview.app).
  // The previous (.*\.)? pattern was overly broad. Anchored to prevent spoofing
  // via domains like osintviewEVIL.vercel.app.
  /^https:\/\/([a-z0-9-]+\.)?osintview\.app$/,
];
