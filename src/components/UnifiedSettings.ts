...
import { clearActivePreset } from '@/services/panel-presets';
...
export class UnifiedSettings {
  ...
  private savePanelChanges(): void {
    ...
    // Manual panel toggle means user is customizing — clear any active preset
    clearActivePreset();
    const presetBar = document.getElementById('presetBar');
    if (presetBar) {
      presetBar.querySelectorAll('.preset-btn').forEach(b => b.classList.remove('active'));
      const customBtn = presetBar.querySelector('[data-preset="custom"]');
