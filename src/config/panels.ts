...
const FULL_PANELS: Record<string, PanelConfig> = {
  ...
  'gdelt-intel': { name: 'Live Intelligence', enabled: false, priority: 1, ...(_desktop && { premium: 'enhanced' as const }) },
  poi: { name: 'Persons of Interest', enabled: true, priority: 1 },
  'intel-graph': { name: 'Intel Link Graph', enabled: true, priority: 1 },
  'missile-tracker': { name: 'Missile / Drone Tracker', enabled: false, priority: 1 },
  'osint-toolkit': { name: 'OSINT Toolkit', enabled: false, priority: 2 },
  ...
  'chokepoint-flow': { name: 'Global Chokepoint Flow', enabled: false, priority: 2 },
  'telegram-osint': { name: 'OSINT Telegram Channels', enabled: false, priority: 2 },
  'supply-chain-prices': { name: 'Consumer Economy Snapshot', enabled: false, priority: 2 },
  'cross-source-signals': { name: 'Cross-Source Signals', enabled: false, priority: 1 },
  'signal-confidence': { name: 'Signal Confidence', enabled: false, priority: 2 },
  'time-travel': { name: 'Time Machine', enabled: false, priority: 2 },
  'auto-brief': { name: 'Auto-Briefs', enabled: false, priority: 2 },
  'hypothesis-generator': { name: 'Hypothesis Generator', enabled: false, priority: 2 },
...
// ============================================
import {
  DEFAULT_PANELS as GODMODE_PANELS,
  DEFAULT_MAP_LAYERS as GODMODE_MAP_LAYERS,
  MOBILE_DEFAULT_MAP_LAYERS as GODMODE_MOBILE_MAP_LAYERS,
} from './variants/godmode';
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  marketsFinance: {
    ...
    panelKeys: ['commodities', 'markets', 'economic', 'trade-policy', 'supply-chain', 'finance', 'polymarket', 'macro-signals', 'gulf-economies', 'etf-flows', 'stablecoins', 'crypto', 'heatmap', 'chokepoint-flow'],
    variants: ['full'],
  },
  topical: {
