...
Sentry.init({
  ...
  release: `osintview@${__APP_VERSION__}`,
  environment: location.hostname === 'osintview.app' ? 'production'
...
// In web production, route RPC calls through api.osintview.app (Cloudflare edge).
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('osintview-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('osintview-beta-mode', 'true');
    else localStorage.removeItem('osintview-beta-mode');
    location.reload();
  },
});
