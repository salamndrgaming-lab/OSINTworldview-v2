...
describe('Bootstrap cache key registry', () => {
  ...
  it('every cache key maps to a handler file or external seed script', () => {
    ...
    const handlerDirs = join(root, 'server', 'osintview');
    const handlerFiles = [];
    function walk(dir) {
...
describe('Bootstrap key hydration coverage', () => {
  it('every bootstrap key has a getHydratedData consumer in src/', () => {
    ...
    const PENDING_CONSUMERS = new Set(['chokepointTransits', 'correlationCards', 'poi', 'telegramNarratives', 'entityGraph', 'hypotheses', 'narrativeDrift', 'crossSourceSignals', 'councilSynthesis']);
    for (const key of keys) {
